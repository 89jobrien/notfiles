...
**Each crate has its own `AGENTS.md`** with the detail you need when editing
...
Per-crate detail lives in each crate's own `AGENTS.md`:

| Read this                     | When you're working on                       |
| ----------------------------- | -------------------------------------------- |
| `crates/notcore/AGENTS.md`    | Shared types, config, the leaf-crate rule    |
| `crates/notfiles/AGENTS.md`   | Linking, state, adding a subcommand          |
| `crates/notsecrets/AGENTS.md` | Provider chain contract, the age implementation |
| `crates/nothooks/AGENTS.md`   | Hook phases and setup-hook state             |
| `crates/notnet/AGENTS.md`     | Tailnet join, auth key chain                 |
| `crates/notstrap/AGENTS.md`   | Bootstrap order and the report failure model |
| `crates/notgraph/AGENTS.md`   | Graph analysis and emitters                  |
| `crates/notforge/AGENTS.md`   | Gitea — read the status note before planning |
